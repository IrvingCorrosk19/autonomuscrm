...
public class DbIntelligenceGraphPageTests
{
    private readonly PostgresWebApplicationFixture _fixture;

    public DbIntelligenceGraphPageTests(PostgresWebApplicationFixture fixture) => _fixture = fixture;

    [Fact]
    public async Task GraphPage_ManagerCanLoad()
    {
        ...
        var page = await client.GetAsync("/DatabaseIntelligence/Graph");
        Assert.Equal(System.Net.HttpStatusCode.OK, page.StatusCode);
        var html = await page.Content.ReadAsStringAsync();
        Assert.Contains("Business graph", html, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("not database tables", html, StringComparison.OrdinalIgnoreCase);
    }
}

[Collection("PostgresWebIntegration")]
[Trait("Category", "DatabaseIntelligence")]
public class DbIntelligenceGraphHubTests : IAsyncLifetime
{
    private readonly PostgresWebApplicationFixture _fixture;
    private readonly List<HubConnection> _connections = [];

    public DbIntelligenceGraphHubTests(PostgresWebApplicationFixture fixture) => _fixture = fixture;
    ...
    [Fact]
    public async Task Admin_CanSubscribeToGraphJobInTenant()
    {
        if (_fixture.SkipReason != null)
            throw new InvalidOperationException(_fixture.SkipReason);

        var (tenantId, jobId) = await SeedGraphJobAsync();
        var conn = await ConnectAsync(await LoginAsync("admin@autonomuscrm.local", "Admin123!", tenantId));
        await conn.InvokeAsync("SubscribeGraphBuildJob", jobId, tenantId);
    }

    [Fact]
    public async Task CrossTenant_SubscribeGraphJob_Rejected()
    {
        if (_fixture.SkipReason != null)
            throw new InvalidOperationException(_fixture.SkipReason);
        ...
        var ex = await Assert.ThrowsAsync<HubException>(() =>
            conn.InvokeAsync("SubscribeGraphBuildJob", jobId, otherTenantId));
        Assert.Contains("Cross-tenant", ex.Message, StringComparison.OrdinalIgnoreCase);
    }

    private async Task<(Guid TenantId, Guid JobId)> SeedGraphJobAsync()
    {
        var factory = _fixture.Factory ?? throw new InvalidOperationException();
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        ...
        db.DbBusinessGraphJobs.Add(job);
    ...
    private async Task<Guid> EnsureSecondTenantAsync(Guid existingTenantId)
    {
        ...
        var tenant = AutonomusCRM.Domain.Tenants.Tenant.Create("DIP Graph Hub B", "Isolation");
