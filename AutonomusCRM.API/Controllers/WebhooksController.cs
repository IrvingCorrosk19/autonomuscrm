...
public class WebhooksController : ControllerBase
{
    ...
    public WebhooksController(IStringLocalizer<SharedResource> localizer) => _localizer = localizer;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (customerId == Guid.Empty)
            return BadRequest(ApiLocalization.Error(_localizer, "Api_Error_CustomerIdRequired"));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (payload.Records == null || payload.Records.Count == 0)
            return BadRequest(ApiLocalization.Error(_localizer, "Api_Error_RecordsRequired"));

        var guard = ImportGuard.ValidateRowCount(payload.Records.Count);
        if (!guard.Ok)
            return BadRequest(ApiLocalization.Error(_localizer, guard.ErrorKey!, guard.FormatArgs ?? Array.Empty<object>()));
