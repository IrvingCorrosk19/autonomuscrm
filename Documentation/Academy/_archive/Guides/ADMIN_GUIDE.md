# Administrador — Guía Operativa AutonomusCRM Academy

> **Programa:** AutonomusCRM Enterprise Academy  
> **Rol:** Administrador  
> **Usuario de práctica:** `admin@autonomuscrm.local`  
...
## Quién eres como Administrador

Administrador operativo del CRM

Configura el CRM para que cada equipo trabaje con datos limpios, procesos claros y automatización confiable.

### Tu impacto en la empresa

```mermaid
flowchart LR
    A[Tú: Administrador] --> B[Ingresos]
...
|-----------|-----------------|
| **Ingresos** | Elimina cuellos de botella (permisos, workflows, datos duplicados) que frenan cierres. |
| **Satisfacción** | Estructura Customer Success, tickets y playbooks para respuestas consistentes. |
| **Crecimiento** | Escala usuarios, integraciones y políticas sin perder control. |
...
#### Reducción tiempo alta usuario 5 días a 4 horas

**Contexto:** RRHH enviaba tickets manuales; errores de rol frecuentes.

**Desafío:** Plantilla y workflow inexistentes.

**Acciones:** Workflow onboarding; checklist rol mínimo; video 5 min.

**Resultado:** Alta mismo día; cero tickets rol incorrecto en Q2.
...
#### Limpieza post-campaña marketing

**Contexto:** 3.000 leads importados; 12% duplicados con clientes.

**Desafío:** Marketing presiona velocidad vs calidad.

**Acciones:** Reglas dedup; asignación automática; reporte calidad semanal.

**Resultado:** SQL rate +22%; quejas ventas -80%.
...
| 2 | Email: `admin@autonomuscrm.local` | Identidad única auditada |
...
- Prioriza `Usuarios` si el tiempo se agota.
...
| | |
|---|---|
| **Qué hacer** | Onboarding de nuevos usuarios |
...
| | |
|---|---|
| **Qué hacer** | Apoyar escalamientos comerciales |
...
## Escenario 4.1 — Nuevo empleado comercial

**Historia:** Contratas SDR sin experiencia en CRM.
...
flowchart TD
    S[Inicio]
    S --> A1[Crear usuario Sales en /Users]
    A1 --> A2[Asignar mentor y tareas onboarding]
    A2 --> A3[Enviar Quick Start]
    A3 --> A4[Validar login día 1]
    A4 --> A5[Revisión KPI día 15]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Crear usuario Sales en `/Users`
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Asignar mentor y tareas onboarding
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Enviar Quick Start
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Validar login día 1
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Revisión KPI día 15
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Productivo en 7 días; primera oportunidad día 10.
...
## Escenario 4.2 — Importación masiva post-evento

**Historia:** Feria IT Expo: 200 tarjetas escaneadas.
...
flowchart TD
    S[Inicio]
    S --> A1[Preparar CSV plantilla]
    A1 --> A2[Import en /Leads]
    A2 --> A3[Deduplicar Customer 360]
    A3 --> A4[Asignar por territorio]
    A4 --> A5[Reportar a Marketing]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Preparar CSV plantilla
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Import en `/Leads`
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Deduplicar Customer 360
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Asignar por territorio
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Reportar a Marketing
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** 200 leads en <2h sin duplicar clientes activos.
...
## Escenario 4.3 — Workflow de asignación leads

**Historia:** Leads inbound sin dueño generan retraso de 48h.
...
flowchart TD
    S[Inicio]
    S --> A1[Mapear trigger en workflow]
    A1 --> A2[Regla round-robin por territorio]
    A2 --> A3[Probar con lead de prueba]
    A3 --> A4[Activar en producción]
    A4 --> A5[Monitorear 72h]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Mapear trigger en workflow
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Regla round-robin por territorio
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Probar con lead de prueba
   - *Verificación:* Registro actualizado y próximo paso con fecha.
...
**Resultado de negocio:** Tiempo asignación <15 min.
...
## Escenario 4.4 — Política ABAC descuentos

**Historia:** Managers aprueban descuentos sin trazabilidad.
...
flowchart TD
    S[Inicio]
    S --> A1[Definir umbral en /Policies]
    A1 --> A2[Trust Studio para >15%]
    A2 --> A3[Capacitar managers]
    A3 --> A4[Auditar primera semana]
    A4 --> A5[Ajustar si fricción]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Definir umbral en `/Policies`
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Trust Studio para >15%
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Capacitar managers
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Auditar primera semana
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Ajustar si fricción
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** 100% descuentos mayores auditados.
...
## Escenario 4.5 — Limpieza duplicados directorio

**Historia:** Auditoría encuentra 23 clientes duplicados.
...
flowchart TD
    S[Inicio]
    S --> A1[Export lista duplicados]
    A1 --> A2[Fusionar en Customer 360]
    A2 --> A3[Actualizar deals vinculados]
    A3 --> A4[Notificar a dueños de cuenta]
    A4 --> A5[Checklist prevención]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Export lista duplicados
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Fusionar en Customer 360
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Actualizar deals vinculados
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Notificar a dueños de cuenta
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Checklist prevención
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Directorio unificado; forecast confiable.
...
## Escenario 4.6 — Ticket CS sin asignar

**Historia:** Cola de soporte con 8 tickets >4h sin dueño.
...
flowchart TD
    S[Inicio]
    S --> A1[Customer Success OS cola]
    A1 --> A2[Regla auto-asignación]
    A2 --> A3[Escalar a Support lead]
    A3 --> A4[SLA comunicado a clientes]
    A4 --> A5[Post-mortem proceso]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Customer Success OS cola
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Regla auto-asignación
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Escalar a Support lead
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. SLA comunicado a clientes
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Post-mortem proceso
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Cola cero en 24h.
...
## Escenario 4.7 — Configuración playbooks CS

**Historia:** Nuevo playbook Renovación 90 días.
...
flowchart TD
    S[Inicio]
    S --> A1[Documentar pasos negocio]
    A1 --> A2[Crear en Customer Success]
    A2 --> A3[Vincular tareas tipo]
    A3 --> A4[Capacitar Support]
    A4 --> A5[Piloto con 3 cuentas]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Documentar pasos negocio
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Crear en Customer Success
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Vincular tareas tipo
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Capacitar Support
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Piloto con 3 cuentas
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Playbook adoptado al 100% en renovaciones.
...
## Escenario 4.8 — Mantenimiento planificado

**Historia:** Actualización de módulo Revenue OS sábado 02:00.
...
flowchart TD
    S[Inicio]
    S --> A1[Ventana comunicada 72h antes]
    A1 --> A2[Backup verificado]
    A2 --> A3[Ejecutar deploy]
    A3 --> A4[Smoke test post-deploy]
    A4 --> A5[All-clear al equipo]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Ventana comunicada 72h antes
   - *Verificación:* Registro actualizado y próximo paso con fecha.
...
5. All-clear al equipo
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Cero tickets por mantenimiento no comunicado.
...
## Escenario 4.9 — Usuario saliente desactivación

**Historia:** Vendedor senior deja la empresa viernes.
...
flowchart TD
    S[Inicio]
    S --> A1[Desactivar en /Users inmediato]
    A1 --> A2[Reasignar deals en /Deals]
    A2 --> A3[Reasignar tareas]
    A3 --> A4[Auditoría accesos últimos 7 días]
    A4 --> A5[Handoff cartera a manager]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Desactivar en `/Users` inmediato
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Reasignar deals en `/Deals`
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Reasignar tareas
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Auditoría accesos últimos 7 días
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Handoff cartera a manager
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Cero acceso post-salida; cartera continua.
...
## Escenario 4.10 — Escalamiento comercial urgente

**Historia:** Sales bloqueado por permiso en propuesta.
...
flowchart TD
    S[Inicio]
    S --> A1[Reproducir con usuario]
    A1 --> A2[Revisar Policies/rol]
    A2 --> A3[Fix mínimo necesario]
    A3 --> A4[Comunicar a solicitante]
    A4 --> A5[Documentar en Audit]
    A5 --> O[Resultado]
```

**Pasos detallados:**

1. Reproducir con usuario
   - *Verificación:* Registro actualizado y próximo paso con fecha.

2. Revisar Policies/rol
   - *Verificación:* Registro actualizado y próximo paso con fecha.

3. Fix mínimo necesario
   - *Verificación:* Registro actualizado y próximo paso con fecha.

4. Comunicar a solicitante
   - *Verificación:* Registro actualizado y próximo paso con fecha.

5. Documentar en Audit
   - *Verificación:* Registro actualizado y próximo paso con fecha.

**Resultado de negocio:** Desbloqueo <1h; sin sobre-permisos.
...
|-----|---------------|-------------|--------------|
| Tiempo de alta de usuario | Definir con manager | Command / Revenue / CS | Tendencia 2 semanas negativa |
| Workflows activos sin errores | Definir con manager | Command / Revenue / CS | Tendencia 2 semanas negativa |
| Tickets CS sin asignar | Definir con manager | Command / Revenue / CS | Tendencia 2 semanas negativa |
| Duplicados en directorio | Definir con manager | Command / Revenue / CS | Tendencia 2 semanas negativa |
| Cumplimiento de políticas | Definir con manager | Command / Revenue / CS | Tendencia 2 semanas negativa |
...
### Escenario IA — Workflow sugerido por IA

Agente propone workflow asignación leads. Pruebas en sandbox antes de aprobar.

**Tu decisión:** Aprobar · Rechazar · Escalar — siempre con nota de negocio.

### Escenario IA — Duplicados detectados

IA lista 15 posibles duplicados. Validas manualmente antes de fusión.
...
**Examen completo:** ver `ROLE_CERTIFICATION_EXAMS.md` — sección **Administrador**.
...
*AutonomusCRM Enterprise Academy — Administrador — Documento generado para capacitación operativa.*
