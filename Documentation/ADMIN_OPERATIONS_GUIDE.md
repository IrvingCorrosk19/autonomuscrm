...
| Facturación | `/billing` | ✅ | ✅* |
| Provisioning tenant | `POST /api/tenants` | ✅ | ❌ |
...
1. **Brecha UI vs API:** `CommercialWriteAuthorizationMiddleware` bloquea escritura comercial UI para `Viewer`/`Support`, pero controllers comerciales API no filtran por rol en todos los endpoints.
...
| Brecha de seguridad | Admin + auditoría `/Audit` |
